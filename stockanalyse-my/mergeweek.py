...
def genweek(stockCode):
	...
	df.to_csv('./data/'+stockCode+'_week.csv')


def calcAllInstructor(stockCode):
	#MACD params is 5,10,7	
	pa = os.getcwd()
	if os.path.exists(pa+'/data/'+stockCode+'_week.csv')==False:
		print stockCode," week data not exists"
		return
	stock_data = pandas.read_csv('./data/'+stockCode+'_week.csv')
	if stock_data['close'].count()==0:
		print stockCode," data empty, maybe exist stock market"
		return
	...
	stock_data.to_csv('./data/'+stockCode+'_weekewma.csv')


	
def calcMacd(stockCode):
	pa = os.getcwd()
	if os.path.exists(pa+'/data/'+stockCode+'_weekewma.csv')==False:
		return 0
	stock_data = pandas.read_csv('./data/'+stockCode+'_weekewma.csv')
