...
def getTodayRecord(stockCode, tday):
	#tday = time.strftime('%Y-%m-%d',time.localtime())
	tfile = './data/'+stockCode+"_"+tday+"_trade.csv"
...
def getTradeRecord(stockCode):
	...
	getTodayRecord(stockCode,x)



def analyseStock(stockCode,day):
	per = 0
	if os.path.exists('./data/'+stockCode+'_ewma.csv')==True:
		szdata = pandas.read_csv('./data/'+stockCode+"_ewma.csv")
...
def analyseHighLowExchange(stcode,t):
	#t = time.strftime('%Y-%m-%d',time.localtime())
	th,tl = getHighLow(stcode)

	avp = (th+tl)*0.5

	stock_data = pandas.read_csv('./data/'+stcode+"_"+t+"_trade.csv")
...
def analyseOneStock(st, stock_data):
    days = []
    #stock_data = pandas.read_csv('./data/'+st+'.csv')
    co = stock_data['close'].count()
    z = range(co-15,co)
    for t in z:
    ...
    analyseHighLowExchange(st,tday)

def showOneStcockPicture(st, stock_data):
    #show picture
    tday = time.strftime('%Y-%m-%d',time.localtime())
    sd = pandas.read_csv('./data/'+st+'_'+tday+'_trade.csv')
    ...
    sd1 = pandas.read_csv('./data/'+st+'_'+day1+'_trade.csv')
    ...
    sd2 = pandas.read_csv('./data/'+st+'_'+day2+'_trade.csv')
...
#st = sys.argv[1]
#st = "SH600606"
tmpStockList = pandas.read_csv('myOwnStock.csv')
#stockCode = tmpStockList.ix[:,,'stockcode']

for st in tmpStockList['stockcode'].values:
    filename = './data/%s.csv'%st
    if os.path.exists(filename):
        stock_data = pandas.read_csv(filename)
        analyseOneStock(st, stock_data)
        showOneStcockPicture(st, stock_data)
    else:
        print st, "not exist"
