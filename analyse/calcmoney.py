...
def calcMoneyForEachStock(stcode, sd):
    co = sd['close'].count()
...
def calcMoneyShowEachStock(stcode):
...
for st in conf.myStlist:
    filename = './data/%s.csv'%st
    if os.path.exists(filename):
        stock_data = pandas.read_csv(filename)
        calcMoneyForEachStock(st, stock_data)
        calcMoneyShowEachStock(st)
    else:
        print st, "not exist"
'''
