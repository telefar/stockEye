...
def printGoodStock(stocks, filter, day):
    ...
    for stock in goodstocks:
        ...
        print ("{0}\t{1}\t{2}\tlast day of data on {3}".format(stock.id, stock.name, stock.score, stock.dates[len(stock.dates) - 1]))
...
def subprocessfunc(data, day):
    printGoodStock(data, tree1filter, day)
    verify(data, tree1filter, day, 0.0)
...
def mystockSelection():
    ...
    logging.basicConfig(filename= conf.select_result+datetime.now().strftime("%Y_%m_%d_%H_%M_%S")+ '.log',level=logging.DEBUG)
    ...
    stocks = getDataFromMongoOnethread(prefixes, startday, endday)
